...
class AuthController {
  static async signup(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (user.role === 'applicant') {
        let activationToken = await AuthService.generateActivationLink(user._id as ObjectId);
  ...
  static async signIn(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let { _id, role, preferred_name, name } = user as { _id: mongoose.Schema.Types.ObjectId; role: string, preferred_name: string, name: string };
      let idStr = _id.toString();
      const token = await generateToken(idStr, role);
      const tokenExp = await decodeToken(token);
      ...
      res.cookie('sessionId', sessionId, {
        ...
        secure: true,
        sameSite: 'none',
        maxAge: (tokenExp as number * 100) - Date.now(),
      });
      let userData = { preferred_name, name, role, idStr };
      const resPayload = { userData };
      await PublicService.addToActiveUsersList(sessionId.generatedId, userData, tokenExp as number);
